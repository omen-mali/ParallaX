...
<html lang="en">
  ...
  <body>
    ...
    <script>
      ...
      const activeDecisions = (snapshot.decisions || []).filter((d) => d.status === "active");
      ...
      const decisionsSection = section("decisions", "decision", "Active decisions");
      ...
      const tasksSection = section("tasks", "task", "Open tasks");
      const taskList = document.createElement("div");
      taskList.className = "list";
      if (openTasks.length === 0) {
        taskList.append(emptyState("No open tasks."));
      } else {
